...
namespace UI.AvansGreenPortal.Controllers
{
    public class PacketController : Controller
    {
        ...
        public IActionResult PacketOverview()
        {
            ...
            {
                vm.CurrentUser = new CurrentUserViewModel() { IsAuthenticated = User.Identity.IsAuthenticated, UserType = User.FindFirst("UserType")?.Value };
        ...
        {
            MyReservationsViewModel vm = new()
            {
                CurrentUser = new CurrentUserViewModel() { IsAuthenticated = User.Identity.IsAuthenticated, UserType = User.FindFirst("UserType")?.Value }
            };
            Student student = _studentRepository.GetByStudentNr(User.Identity.Name);
        ...
        {
            PacketDetailViewModel vm = new()
            {
                Packet = _packetRepository.GetById(Id)
            };
            if (User.Identity != null)
            {
                vm.CurrentUser = new CurrentUserViewModel() { IsAuthenticated = User.Identity.IsAuthenticated, UserType = User.FindFirst("UserType")?.Value };
                if (vm.CurrentUser.UserType is "CanteenEmployee")
                {
                    CanteenEmployee canteenEmployee = _canteenEmployeeRepository.GetByEmployeeNr(User.Identity.Name);
        ...
        {
            var currentUser = new CurrentUserViewModel() { IsAuthenticated = User.Identity.IsAuthenticated, UserType = User.FindFirst("UserType")?.Value };
            try
            {
            ...
            catch (Exception e)
            {
                ...
                return View("PacketDetail", new PacketDetailViewModel() { Packet = _packetRepository.GetById(Id), CurrentUser = currentUser });
            }
            return RedirectToAction("MyReservations", new MyReservationsViewModel() { CurrentUser = currentUser });
        }
        ...
        public IActionResult EditPacket(int Id)
        {
            ...
            if (packet.StudentId.HasValue)
            {
                ...
                return View("PacketDetail",
                    new PacketDetailViewModel()
                    {
                        Packet = _packetRepository.GetById(Id),
                        CurrentUser = new CurrentUserViewModel() { IsAuthenticated = User.Identity.IsAuthenticated, UserType = User.FindFirst("UserType")?.Value }
                    });
